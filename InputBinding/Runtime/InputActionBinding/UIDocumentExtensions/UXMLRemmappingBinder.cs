    ...
    public class UXMLRemmappingBinder : MonoBehaviour
    {
        ...
        public class InputBind
        {
            ...
            public void UpdateButton()
            {
                ...
                if (_button != null && bindingPath != null && bindingPath != string.Empty)
                {
                    //Find the icon for this binding path
                    int endIndex = bindingPath.IndexOf('>');
                    string catergoryPath = bindingPath.Substring(1, endIndex - 1);

                    InputBindingCategories category;
                    switch (catergoryPath)
                    {
                        case "Gamepad":
                            category = InputBindingCategories.Gamepad;
                            break;
                        case "Keyboard":
                            category = InputBindingCategories.Keyboard;
                            break;
                        case "Mouse":
                            category = InputBindingCategories.Mouse;
                            break;
                        default:
                            category = InputBindingCategories.Gamepad;
                            break;
                    }
                    foundTexture = ButtonIconBindingsEditorSettings.BindVisualElementImageToIconBinding(_icon, $"{action.bindings[bindingIndex].effectivePath}", category);
                }
